...
var Event = function (name, target) {
  ...
  // add listener
  this.add = function(type, callBack, data, context){
    var exist = false;
    var data = data || {};
    var context = context || {};
    ...
    if(!exist)
      this._listeners.push({type:type, callBack:callBack, data:data, context:context});
    //
  }
  ...
  this.dispatch = function(type, args){
    ...
    for (var i = 0; i < this._listeners.length; i++)
    {
      if(type == this._listeners[i].type)
      {
        ...
        // agrega a data customizada
        for(var j in this._listeners[i].data)
          data[j] = this._listeners[i].data[j]
        //
